...
Line differential protection compares the currents measured at the ends of a protected line. Because the line is a series element with no intended in-zone tap-off, the current entering the zone at one end equals the current leaving it at the other under normal load and external-fault conditions, so the vector sum of the correctly referred currents is ideally zero. An internal fault unbalances this sum and produces a differential current that indicates a fault within the protected zone. The participating #acrpl("CT") are installed at the different line ends, which are generally separated geographically, so the measured currents are exchanged between the line-end #acrpl("IED") over a communication channel before the differential quantity is formed. #cite(<RED670_Application_Manual>), #cite(<Mustajärvi_Instrument_Transformer>)

In contrast to transformer differential protection, line differential protection does not require compensation for a transformer vector group, because the protected line changes neither the voltage level nor the phase and the currents at the two ends therefore refer to the same primary quantity. The #acrpl("CT") at the different ends may nevertheless have different ratios and magnetic characteristics, and this mismatch, together with the line charging current, produces a small standing differential current that the protection must accommodate without operating. #cite(<RED670_Application_Manual>), #cite(<IEC_61869_100>)

As with every differential principle, the dimensioning condition is the external through fault. If the #acrpl("CT") at the two ends saturate unequally under a high through-fault current, their secondary currents no longer correspond even though the fault lies outside the protected zone, and the resulting current difference can appear to the relay as a false differential current. A restrained characteristic is therefore applied so that the required operating current increases with the current passing through the zone, which supports stability during external faults while retaining sensitivity to internal faults. #cite(<RED670_Application_Manual>), #cite(<IEC_61869_100>)

The RED670 application manual states its #acr("CT") requirement as a rated equivalent limiting secondary #acr("emf") $E_"al"$ that the selected #acr("CT") must satisfy through $E_"al" >= E_"al,req"$. The required value is derived from the relevant fault current, the #acr("CT") ratio, the secondary-loop resistance and a device-specific dimensioning factor, in the same form used for the transformer-differential requirement but without the additional through-fault multiplier associated with the two-transformer arrangement. This required #acr("emf") is the quantity against which #acr("SECP") is compared for the line-differential case in the assessment chapters. #cite(<RED670_Application_Manual>), #cite(<IEC_61869_2>), #cite(<IEC_61869_100>)
...
For PX and PXR #acrpl("CT"), the excitation characteristic, secondary winding resistance, burden resistance and turns ratio provide the information required to assess performance in relation to the associated protection system. #cite(<IEC_61869_2>)
...
Remanence is the magnetic flux that remains in the #acr("CT") core after the magnetising excitation has been removed. Because existing remanent flux reduces the remaining flux margin in one direction, it can lead to earlier saturation during a subsequent fault. #cite(<IEC_61869_2>) differentiates classes with and without a specified remanent-flux limit. Classes PR, PXR and TPY include a remanence limit, whereas classes P, PX and TPX do not. Class TPZ is specified through a secondary time constant and the alternating component of the instantaneous error. #cite(<IEC_61869_100>)

The #acr("CT") classes consequently represent different methods of specifying protection performance. Classes P and PR are defined through composite-error requirements under symmetrical steady-state short-circuit conditions. Classes PX and PXR are specified through the excitation characteristic and associated circuit parameters. Classes TPX, TPY and TPZ are defined in relation to transient error under asymmetrical short-circuit conditions. Selection among these classes must therefore follow the protection principle, expected fault conditions, burden and applicable protection-device requirements. #cite(<IEC_61869_100>), #cite(<IEC_61869_2>)

== Transient Dimensioning of Protective #acrpl("CT")

The parameters introduced in the preceding section describe #acr("CT") behaviour under symmetrical steady-state conditions. A real short-circuit current generally contains a decaying direct-current component whose magnitude depends on the instant of fault inception and on the primary time constant of the network. This component adds a unidirectional contribution to the secondary linked flux and can therefore drive the core into saturation at a current considerably below the value derived from the alternating component alone. #cite(<IEC_61869_2>), #cite(<IEC_61869_100>)

#cite(<IEC_61869_2>) addresses this behaviour through a dedicated set of dimensioning quantities: the rated symmetrical short-circuit current factor, the transient factor, the transient dimensioning factor and the rated equivalent limiting secondary electromotive force. Together these express the required core performance as a single voltage requirement that can be compared with the capability of an actual #acr("CT").
...
It characterises the symmetrical component of the fault current only and is in this respect the transient-performance counterpart of the accuracy limit factor used for classes P and PR. #cite(<IEC_61869_2>)

The transient requirement is completed by the specified primary time constant $T_p$ and by the specified duty cycle. For a single energisation, the duty cycle is defined through the time to accuracy limit in the first fault $t'_"al"$. For an auto-reclosing sequence, the duration of the first fault $t'$, the fault repetition time $t_"fr"$ and the time to accuracy limit in the second fault $t''_"al"$ are specified in addition. #cite(<IEC_61869_2>), #cite(<IEC_61869_100>)
