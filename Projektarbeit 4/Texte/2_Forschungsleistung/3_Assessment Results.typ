...
Two sources of evidence are used: the comparison workbook @Excel_Calc, with ten transformer-differential and ten line-differential cases alongside their #acr("SECP") counterparts, and a #acr("SECP") detailed calculation report, which exposes the formulae, intermediate values and normative references actually printed. Releasability is decided by the non-compensatory decision rule; the weighted decision matrix contained in the workbook is retained only as a supplementary indicator and does not by itself determine the verdict.
...
The two procedures are physically related: #acr("IEC") 61869-2 defines both the accuracy-limit quantities of class P and PR #acrpl("CT") and the equivalent limiting #acr("emf"), and the REF650 guide notes that the secondary limiting #acr("emf") $E_"ALF"$ of a class P or PR #acr("CT") is approximately equal to $E_"al"$. They are nonetheless not directly comparable in the form each manual presents them, as summarised in @tab-oc-comparison.
...
The generated report has the standardised structure already described for the #acr("SECP") process: a Microsoft Word document ending, per application, in a tap-level summary. In layout and completeness it is suitable as an auditable design record — the customer receives the inputs, the derivation and the verdict in a single consistent document.

Two substantive issues nevertheless limit its fitness for delivery, and both are engineering issues rather than cosmetic ones. The first is the transformer-differential formula defect of RQ1 and RQ2: the report presents an incorrect required value as an authoritative, standard-referenced result, and a customer auditing the document against the RET670 manual would find that the printed formula does not match the cited clause. The second is the outdated normative reference discussed below. Neither can be remedied by a per-report manual workaround within the effort budget of the decision rule, because both originate in the tool rather than in the individual report; they are properties of every report #acr("SECP") generates for the affected function, and are therefore treated as improvement-required findings for the transformer-differential case rather than as documentable limitations.

An internal inconsistency between an inline “adequate” statement and a “not adequate” summary was reported for an earlier #acr("SECP") example. It is not reproduced in the current report, whose inline verdicts and tap summaries are mutually consistent throughout, and is therefore regarded as pertaining to a superseded revision and not carried forward as a finding of this evaluation.
