...
Numerical correctness is assessed by comparing the required equivalent limiting secondary #acr("emf") computed by #acr("SECP") with the value computed by the independent reference model for the same input set. For each case two criteria are evaluated, the governing value is taken as the larger of the twoand the resulting pass/fail decision is compared in addition to the numerical values. The available dataset covers configurations at 220 kV, 150 kV and 33 kV. The assessment is limited to these operating points; the supplied material does not establish that they span the complete range of applications processed in #acr("SECP").
