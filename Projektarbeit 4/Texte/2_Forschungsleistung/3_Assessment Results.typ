...
*Answer to RQ1.* Within the dataset, #acr("SECP") preserves the final suitability decision in all twenty compared cases. The line-differential results agree closely; the transformer-differential results deviate by a mean of approximately 8.79% and up to 21.98%, caused by an isolated formula defect that counts the lead resistance twice. The deviation is conservative and flips no decision in the dataset. Still, the reported required value is numerically incorrect and the absence of a decision change does not guarantee that a case close to the threshold would be unaffected. For this reason, numerical accuracy and decision consistency are reported separately: the transformer-differential calculation is decision-consistent but not numerically correct.
...
*Equivalence assessment.* The two procedures are physically related: #acr("IEC") 61869-2 defines both the accuracy-limit quantities of class P and PR #acrpl("CT") and the equivalent limiting #acr("emf") and the REF650 guide notes that the secondary limiting #acr("emf") $E_"ALF"$ of a class P or PR #acr("CT") is approximately equal to $E_"al"$. In the form each manual presents them, however, they are not directly comparable, as summarised in @tab-oc-comparison.
...
However, two substantive issues limit its fitness for delivery and both are engineering issues rather than cosmetic ones. The first is the transformer-differential formula defect of RQ1 and RQ2: the report presents an incorrect required value as an authoritative, standard-referenced result. A customer auditing the document against the RET670 manual would find that the printed formula does not match the cited clause. The second is the outdated normative reference discussed below. Neither can be remedied by a per-report manual workaround within the effort budget of the decision rule, because both originate in the tool rather than in the individual report. They are properties of every report #acr("SECP") generates for the affected function and are therefore treated as improvement-required findings for the transformer-differential case, not as documentable limitations.
