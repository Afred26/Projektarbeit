...
#acr("SECP") returns higher required values than the reference in all ten cases. The deviation of the governing criterion ranges from approximately 2.8% to 28.2%, with an average deviation of approximately 10%. The suitability classification is nevertheless identical in every case. Both methods pass all ten, so the deviation changes no suitability decision in the dataset.
