...
The main research question asked whether the #acr("CT") adequacy calculations produced by #acr("SECP") Global can be released to the customer for the planning-phase use case, under which limitations and, if not, which improvements are required. The answer is conditional and function-specific. For line differential protection the calculations can be released, subject only to disclosure of the small conservative deviation. For transformer differential protection they cannot be released as generated: although the suitability decisions are safe because the deviation is conservative, the required value is numerically incorrect through a lead-resistance double-count, and the required improvement is to apply the lead resistance once in the criterion. For overcurrent protection they cannot be released with the required device at all, because the REF650 is not selectable and the equivalence of the REF630 substitute is not established; the required improvement is either to add the REF650 or to demonstrate the substitute's equivalence.
