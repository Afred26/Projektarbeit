...
Because the Excel calculation serves as the independent reference, it must be verified as well: agreement between two implementations does not demonstrate that either correctly represents the underlying basis. The workbook's separation of inputs, intermediate calculations, criteria and decision allows several values to be reproduced directly from the inputs, namely the available limiting #acr("emf"), the total secondary burden, the transformer rated and through-fault currents, the governing-criterion selection and the final comparison. These can be checked against the P and PR limiting-#acr("emf") relationship of #acr("IEC") 61869-100 and the secondary-loop-resistance definition of #acr("IEC") 61869-2. The device-specific equations have since been identified as Equation 573 and Equation 574 of the RET670 manual, with the cable resistance following @eq-cable, all reproducing the workbook outputs exactly.
