...
The independent calculation is implemented in Microsoft Excel. The workbook separates the input data, intermediate quantities, protection-specific criteria, #acr("CT") capability, suitability decision and comparison with #acr("SECP"). This structure enables the calculation to be traced from the original parameters to the final result.
...
The final suitability decision is obtained by comparing the available #acr("CT") capability with the governing required value. The #acr("CT") is classified as suitable if the available equivalent limiting secondary e.m.f. is equal to or greater than the required value. Otherwise, the #acr("CT") is classified as unsuitable.

The individual calculation sheets follow the same sequence:

- calculation of the cable-loop resistance and total secondary-circuit resistance,
- calculation of the available #acr("CT") limiting secondary #acr("emf"),
- calculation of the application-specific currents,
- calculation of the two required limiting-#acr("emf") criteria,
- selection of the governing criterion,
- comparison of the available and required values,
- assignment of a pass or fail result.
