...
#acr("SECP") is an offline application combining several substation-engineering calculation modules, including conductor design and loading, lightning protection, #acr("CT") and #acr("VT") adequacy checks, battery and charger sizing, auxiliary-transformer sizing and neutral-grounding-resistor sizing, with support for both SI and Imperial units. It is organized around project files (extension `.secp.proj`) that can be created, opened, copied, imported or exported. Data relevant to several calculations can be entered once and reused, which reduces the engineering effort. #acr("CT") calculations form part of the combined #acr("CT") and #acr("VT") adequacy-check module, which assesses the suitability of instrument transformers for selected protection or metering applications.
...
The overall workflow is summarized in @CT_SECP. The calculation starts with the project and system data, followed by the #acr("CT"), cable and relay parameters. Based on these inputs, SECP determines the secondary-loop resistance and performs the calculation required for the selected protection function. The required #acr("CT") performance is then compared with the available #acr("CT") capability to obtain the final suitability decision.

Each calculation stage depends on the results of the preceding stage, so an error in the processing of an input quantity, such as the secondary cable resistance, propagates through the application-specific calculation and affects the reported #acr("CT") requirement. @CT_SECP therefore also provides the process structure used later to locate identified deviations within the SECP calculation chain.
...
#acr("SECP") generates standardized summary and detailed reports as editable Microsoft Word files. A detailed report contains the input data, calculation steps, formulae and results; a summary report contains inputs and final results only. Before generation the user selects the scenario and element, the number of decimal digits and whether unselected elements are summarized. The detailed report follows a consistent structure: cover page and table of contents, project and document information, the referenced standards and nomenclature, then the calculation organized by scenario, bay, core, application and tap. Each application closes with a summary that consolidates the principal #acr("CT") data and results (currents, class, #acr("ALF"), proposed and calculated knee-point voltages, burden and adequacy).
