...
The credibility of the conclusions is discussed through the three validity aspects of case study research #cite(<Runeson_Case_Study>). Construct validity concerns whether the criteria capture what releasability means to the organization. It is addressed by stating the intended use explicitly, deriving the releasable properties from an established quality model and fixing the evaluation criteria before the assessment. 

Internal validity concerns whether an observed deviation is genuinely attributable to #acr("SECP") rather than to the reference calculation or an input error. It is addressed by deriving the reference calculation from the device manuals and the #acr("IEC") 61869 series, by manually reproducing selected reference results and by entering identical, documented inputs into both calculation paths.
