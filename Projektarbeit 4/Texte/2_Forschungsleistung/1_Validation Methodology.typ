...
The verdict is governed by a decision rule that was fixed before the assessment, so that all findings are judged by the same standard. It distinguishes three outcomes: _Yes_, the report is correct, complete, traceable and presentable without intervention. _Yes, with limitation_, the report becomes releasable only after a manual workaround that yields a very good result within approximately 15 to 30 minutes per report; this effort bound is referred to as the workaround budget and the workaround is documented as the limitation. _No, improvement required_, when no workaround satisfies both the quality and the effort condition or no workaround is possible.

A criterion that only degrades the presentation is treated as a limitation, whereas one that affects the engineering result is a blocker. The rule is non-compensatory: A blocker in any sub-question prevents release regardless of how well the other criteria are met. This distinction matters especially in the planning context, since an undetected result error propagates into specification and procurement, so correctness and traceability defects weigh more heavily than cosmetic ones.
...
Every finding of these examinations is recorded through a common three-level classification scheme: The cause level classifies the software defect using the standard classification for software anomalies #cite(<IEEE_1044>), the effect level expresses the resulting numerical or metrological deviation through @eq-deviation and its uncertainty #cite(<JCGM_100_GUM>) and the benchmark level evaluates the consequence against the #acr("CT") adequacy criterion of the #acr("IEC") 61869 series #cite(<IEC_61869_2>), #cite(<IEC_61869_100>). Each finding is additionally assigned a severity that maps directly onto the blocker-versus-limitation distinction of @Decision_Rule, so a finding's classification and its effect on the verdict always remain consistent.

== Validity Considerations

The credibility of the conclusions is discussed through the four validity aspects of case-study research #cite(<Runeson_Case_Study>). Construct validity concerns whether the criteria capture what releasability means to the organization. It is addressed by defining the intended use explicitly, deriving the releasable properties from an established quality model and fixing the decision rule in advance. Internal validity concerns whether an observed deviation is genuinely attributable to #acr("SECP") rather than to the reference calculation or an input error. It is addressed by deriving the reference calculation from the primary standards, cross-checking it and entering identical, documented inputs into both. External validity is limited, because only three application–device cases and one tool version are examined. 

The verdict is therefore reported per case and the cases are chosen to represent the dominant planning applications. Reliability is addressed by documenting the reference calculation, the input sets, the decision rule and the classification scheme, so that the evaluation can in principle be repeated independently.
