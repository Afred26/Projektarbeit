...
The rated accuracy-limit primary current in @eq-kalf is the primary-current value up to which the #acr("CT") complies with the applicable composite-error requirements. #cite(<international_electrotechnical_commission_iec_2012>)

The knee-point electromotive force is relevant particularly to classes PX and PXR. #cite(<international_electrotechnical_commission_iec_2012>) identifies it as the lower limit of the knee-point e.m.f. and relates it to the dimensioning factor $K_x$, secondary winding resistance, rated burden resistance and rated secondary current:

$ E_k = K_x (R_"ct" + R_b) I_"sr" $ <eq-ek>

For PX and PXR #acrpl("CT"), the excitation characteristic, secondary winding resistance, burden resistance and turns ratio provide the information required to assess performance in relation to the associated protection system. #cite(<international_electrotechnical_commission_iec_2012>)

Saturation denotes the operating region in which the #acr("CT") core can no longer reproduce increasing primary current proportionally at the secondary terminals. Fault-current asymmetry and its decaying direct-current component can increase the magnetic flux beyond that produced by the symmetrical alternating component alone. #cite(<international_electrotechnical_commission_iec_2017-2>) therefore uses the time to accuracy limit, the transient factor and the transient dimensioning factor to describe and determine the required transient #acr("CT") performance.

Remanence is the magnetic flux that remains in the #acr("CT") core after the magnetising excitation has been removed. Because existing remanent flux reduces the remaining flux margin in one direction, it can lead to earlier saturation during a subsequent fault. #cite(<international_electrotechnical_commission_iec_2012>) differentiates classes with and without a specified remanent-flux limit. Classes PR, PXR and TPY include a remanence limit, whereas classes P, PX and TPX do not. Class TPZ is specified through a secondary time constant and the alternating component of the instantaneous error. #cite(<international_electrotechnical_commission_iec_2017-2>)

The #acr("CT") classes consequently represent different methods of specifying protection performance. Classes P and PR are defined through composite-error requirements under symmetrical steady-state short-circuit conditions. Classes PX and PXR are specified through the excitation characteristic and associated circuit parameters. Classes TPX, TPY and TPZ are defined in relation to transient error under asymmetrical short-circuit conditions. Selection among these classes must therefore follow the protection principle, expected fault conditions, burden and applicable protection-device requirements. #cite(<international_electrotechnical_commission_iec_2017-2>), #cite(<international_electrotechnical_commission_iec_2012>)

== Transient Dimensioning of Protective #acrpl("CT")

The parameters introduced in the preceding section describe #acr("CT") behaviour under symmetrical steady-state conditions. A real short-circuit current generally contains a decaying direct-current component whose magnitude depends on the instant of fault inception and on the primary time constant of the network. This component adds a unidirectional contribution to the secondary linked flux and can therefore drive the core into saturation at a current considerably below the value derived from the alternating component alone. #cite(<international_electrotechnical_commission_iec_2012>), #cite(<international_electrotechnical_commission_iec_2017-2>)

#cite(<international_electrotechnical_commission_iec_2012>) addresses this behaviour through a dedicated set of dimensioning quantities: the rated symmetrical short-circuit current factor, the transient factor, the transient dimensioning factor and the rated equivalent limiting secondary electromotive force. Together these express the required core performance as a single voltage requirement that can be compared with the capability of an actual #acr("CT").
...
It characterises the symmetrical component of the fault current only and is in this respect the transient-performance counterpart of the accuracy limit factor used for classes P and PR. #cite(<international_electrotechnical_commission_iec_2012>)

The transient requirement is completed by the specified primary time constant $T_p$ and by the specified duty cycle. For a single energisation, the duty cycle is defined through the time to accuracy limit in the first fault $t'_"al"$. For an auto-reclosing sequence, the duration of the first fault $t'$, the fault repetition time $t_"fr"$ and the time to accuracy limit in the second fault $t''_"al"$ are specified in addition. #cite(<international_electrotechnical_commission_iec_2012>), #cite(<international_electrotechnical_commission_iec_2017-2>)

=== Transient Factor and Transient Dimensioning Factor

The transient factor $K_"tf"$ is the ratio of the secondary linked flux at a specified instant of the duty cycle to the peak value of its alternating component. It is defined as a function of time and depends on the primary time constant, the secondary loop time constant $T_s$, the duty cycle and the fault inception angle. #cite(<international_electrotechnical_commission_iec_2012>)

The transient dimensioning factor $K_"td"$ accounts for the increase of the secondary linked flux caused by the direct-current component of the primary short-circuit current. Whereas $K_"tf"$ is time-dependent, $K_"td"$ is the definitive dimensioning parameter and appears on the rating plate. It is obtained either from the #acr("CT") requirements stated by the protection-device manufacturer, which are themselves derived from relay stability type tests, or from a worst-case evaluation of the $K_"tf"$ curves. #cite(<international_electrotechnical_commission_iec_2012>), #cite(<international_electrotechnical_commission_iec_2022>)

#cite(<international_electrotechnical_commission_iec_2017-2>) emphasises that the choice of $K_"td"$ should not rest on analytical formulae alone but should additionally be supported by relay tests using saturated #acr("CT") models. The applicable value therefore depends on both the protection function and the specific protection device, which is directly relevant wherever a calculation tool assigns a fixed factor to an entire application category.
...
where $R_b$ is the rated resistive burden. The sum $R_"ct" + R_b$ constitutes the secondary loop resistance $R_s$. #cite(<international_electrotechnical_commission_iec_2012>)

$E_"al"$ thus expresses the transient requirement as a voltage. This is the form in which modern numerical protection devices state their #acr("CT") requirements, and consequently the form in which the adequacy assessment is ultimately performed. #cite(<mustajarvi_instrument_2016>)

=== Comparison of the Class-Specific Dimensioning Approaches

#cite(<international_electrotechnical_commission_iec_2017-2>) identifies three approaches for defining protective #acrpl("CT"), each of which may result in the same physical implementation. For each approach an e.m.f. at limiting conditions is defined. For classes P and PR:
